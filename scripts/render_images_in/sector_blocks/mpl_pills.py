...
def limit_pct_from_row(row: Dict[str, Any], default_pct: float = 10.0) -> float:
    """
    Read limit band percent from row:
...
def draw_pill_after_text(
    ...
    *,
    ...
    px_to_data_dx_fn,           # callable(ax, px, y_data=y) -> dx
    pad: float = 0.14,
    try_font_deltas: Tuple[int, ...] = (0, -2, -4, -6),
    fallback_y: Optional[float] = None,
) -> bool:
    """
    Draw pill immediately after the given text, but NOT beyond x_right_limit.
    - Will try smaller fonts to fit.
    - If cannot fit and fallback_y provided, draw on fallback_y instead.
    Returns True if drawn, else False.
    """
    if not pill_text:
        return False

    # where does the pill start?
    ...
    if x0 >= x_right_limit:
        if fallback_y is not None and fallback_y != text_y:
            return draw_pill_after_text(
                ...
                pad=pad,
                try_font_deltas=try_font_deltas,
                fallback_y=None,
            )
        return False
