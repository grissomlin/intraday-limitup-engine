...
def draw_block_table(
    ...
    *,
    ...
    if time_note:
        ...
        if len(lines) >= 2:
            line_gap = 0.030 + max(0.0, (subtitle_fs - 26)) * 0.0006  # bigger font => slightly bigger gap
            ax.text(
                0.5,
                header_subtitle_y - line_gap,
                lines[1],
                ha="center",
                va="top",
    ...
    _shadow_text(
        x_left, top_title_y, top_title_left,
        fontsize=box_title_fs, color=fg, ha="left", va="top", weight="bold", alpha=0.98
    )

    bottom_title_text = _t(lang, "box_title_bottom", "Same sector (not locked/touched/10%+)")
    _shadow_text(
        x_left, bot_title_y, bottom_title_text,
        fontsize=box_title_fs, color=fg, ha="left", va="top", weight="bold", alpha=0.95
    )
    ...
    y_start_top, row_h_top = calc_rows_layout(top_rows_area_y_top, top_rows_area_y_bottom, int(rows_per_page), two_line=two_line)
    bot_rows_layout_n = int(rows_per_page) + 1
    y_start_bot, row_h_bot = calc_rows_layout(bot_rows_area_y_top, bot_rows_area_y_bottom, bot_rows_layout_n, two_line=two_line)
    ...
    if top_is_empty:
        ...
        x_right_for_pills = x_tag - px_to_data_dx(ax, safe_gap_to_right_px + BADGE_RIGHT_PAD_PX + ret_reserved_px, y_start_top)
        ...
        for i in range(n):
            ...
            draw_limit_pill_after_text(
                ax, fig,
                x_text=x_name, y=y1, display_text=display_line1,
                row=r, theme=theme,
                x_right_limit=x_right_for_pills,
                fs=pill_fs, gap_px=pill_gap_px,
            )

            if two_line and line2:
                ax.text(
                    x_name, y2,
                    _ellipsize(line2.replace("|", " ").replace("｜", " "), 34),
                    ha="left", va="center",
                    fontsize=row_name_fs, color=sub, weight="bold", alpha=0.95
                )

            status = _safe_str(r.get("limitup_status") or "").lower()
            x_badge = x_tag - px_to_data_dx(ax, BADGE_RIGHT_PAD_PX, y1)
            ...
            x_ret_draw = x_tag - px_to_data_dx(ax, RET_RIGHT_PAD_PX, y2)
            ax.text(
            ...
            if i < n - 1:
                ax.plot([0.06, 0.94], [y_center - row_h_top * 0.50, y_center - row_h_top * 0.50], color=divider, linewidth=1, alpha=0.55)
    ...
    if not peers:
        ...
        for i in range(n2):
            ...
            ret = _safe_float(r.get("ret") or 0.0, 0.0)
            ret_text = _safe_str(r.get("ret_text") or "") or _fmt_ret_pct(ret)
            x_ret_draw = x_tag - px_to_data_dx(ax, RET_RIGHT_PAD_PX, y1)
            ax.text(
            ...
            # ✅ peer pill same line after name; avoid ret block; if too tight => fallback to y2
            ret_w_px = text_width_px(ax, fig, ret_text, ret_fs, weight="bold")
            x_ret_left = x_ret_draw - px_to_data_dx(ax, ret_w_px + 10.0, y1)

            x_right_for_pills_peer = min(
                x_ret_left - px_to_data_dx(ax, 10.0, y1),
                x_tag - px_to_data_dx(ax, RET_RIGHT_PAD_PX + safe_gap_to_right_px, y1),
            )
            ...
            draw_limit_pill_after_text(
                ...
                fs=pill_fs_peer, gap_px=pill_gap_px,
                fallback_y=fallback_line,
            )

            # ✅ peer second line (prev session) now will show (because cli.py filled line2)
            if line2:
                ax.text(
                    x_name, y2,
                    _ellipsize(line2.replace("|", " ").replace("｜", " "), 34),
            ...
            if i < n2 - 1:
                ax.plot([0.06, 0.94], [y_center - row_h_bot * 0.50, y_center - row_h_bot * 0.50], color=divider, linewidth=1, alpha=0.55)
