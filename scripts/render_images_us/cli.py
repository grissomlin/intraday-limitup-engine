...
# =============================================================================
# ✅ NEW: ordered list.txt writer (fix "still not applied" feeling)
# =============================================================================
def write_list_txt_ordered(
    ...
    *,
    ...
    """
    Write list.txt in EXACT desired order:
      1) overview_paths (as returned by render_overview_png)
      2) sector_paths   (as generated by sector loop order + page order)
      3) any remaining *.{ext} in outdir not yet included (alphabetical), safety net

    Each line is RELATIVE filename (JP/KR style).
    """
...
def main() -> int:
    ...
    if debug_on:
        ...
        print(
            f"ymd={payload.get('ymd')} ymd_effective={payload.get('ymd_effective')} generated_at={payload.get('generated_at')}"
        )
        stats = payload.get("stats") or {}
        filters = payload.get("filters") or {}
        print(
            f"stats.snapshot_main_count={stats.get('snapshot_main_count')} snapshot_open_count={stats.get('snapshot_open_count')}"
        )
    ...
    # -------------------------------------------------------------------------
    # 1) Overview first (capture BOTH: paths + _overview_sector_order)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 2) Sector pages (capture sector_page_paths in rendered order)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 3) ✅ list.txt ordered (overview_paths + sector_page_paths)
    # -------------------------------------------------------------------------
    ...
    if debug_on:
        try:
            ...
            print("[US][DEBUG] list.txt first 20 lines:\n" + "\n".join(head20))
        except Exception:
            pass
