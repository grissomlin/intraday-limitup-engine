# scripts/render_images_fr/cli.py
# -*- coding: utf-8 -*-
...
def pick_universe(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    FR snapshot fallback order (open_limit market): prefer snapshot_open.
    """
...
def main() -> int:
    ...
    ap.add_argument(
        "--outdir",
        default=None,
        help="output dir (optional). If omitted -> media/images/fr/{ymd}/{slot}",
    )
    ...
    print(f"[FR] payload={args.payload}")
    print(f"[FR] ymd={ymd or 'UNKNOWN'} slot={slot} outdir={outdir}")
    print(f"[FR] debug={'ON' if debug_on else 'OFF'}")
    ...
    if not args.no_overview:
        ...
        try:
            ...
            print(f"[FR][WARN] overview skipped due to import/render error: {e}", flush=True)
    ...
    for k in ordered_norm_keys:
        ...
        for i in range(total_pages):
            ...
            fname = f"fr_{safe_sector}_p{i+1}.png"
