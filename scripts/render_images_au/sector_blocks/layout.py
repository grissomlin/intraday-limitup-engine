# scripts/render_images_au/sector_blocks/layout.py
# -*- coding: utf-8 -*-
...
class LayoutSpec:
    ...
    header_subtitle_line2_y: float = 0.870  # ✅ 新增：第二行副標題位置（對齊 US）

    footer_y1: float = 0.040
    footer_y2: float = 0.020

    # ✅ 若未來真的要放兩行副標題，可把 top_box_y0 微調下移（US 有註解）
    #    但目前 AU draw 只用一行 time_note，所以數值保持與你原 AU 相同即可
    ...
    title_fs: int = 62
    subtitle_fs: int = 30
    subtitle_line2_fs: int = 26  # ✅ 新增：第二行副標題字體大小（對齊 US）
    page_fs: int = 26
...
PRESETS: Dict[str, LayoutSpec] = {
    "au": LayoutSpec(two_line=True),
    "uk": LayoutSpec(two_line=True),
