...
# =============================================================================
# ✅ NEW: list.txt writer that respects desired order
# =============================================================================
...
# =============================================================================
# ✅ robust overview sector order extractor (handles typo key)
# =============================================================================
...
def _jp_sector_key(sec: str) -> str:
    ...
    - if it returns empty, fallback to casefold
    """
    s = _safe_str(sec)
    if not s:
        return ""
...
def build_limitup_by_sector_jp(universe: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for r in universe:
        ...
        if is_true:
            streak_display = max(1, streak_raw if (prev_locked and streak_raw > 0) else 1)
        else:
            streak_display = 0
        ...
        if is_true:
            badge = "ストップ高"
            line2 = (f"S高張り付き ｜ {streak_display}連 ｜ {ytxt}" if (streak_display > 1 and prev_locked) else f"S高張り付き ｜ {ytxt}")
            status = "hit"
        elif touch_only:
            badge = "一時S高"
...
def main() -> int:
    ...
    outdir = Path(args.outdir) if args.outdir else (REPO_ROOT / "media" / "images" / "jp" / ymd / slot)
    ...
    # -------------------------------------------------------------------------
    # 0) Overview first + capture overview sector order + overview paths
    # -------------------------------------------------------------------------
    ...
    if not args.no_overview:
        try:
            ...
            overview_sector_keys = _extract_overview_order_any(payload_for_overview) or _extract_overview_order_any(payload)

            print("[JP][DEBUG] raw _overview_sector_order exists?:", isinstance(payload_for_overview.get("_overview_sector_order"), list))
            print("[JP][DEBUG] raw order head:", (payload_for_overview.get("_overview_sector_order") or [])[:20])
            if overview_sector_keys:
                met_eff = str(payload_for_overview.get("_overview_metric_eff") or "").strip()
                print(f"[JP] overview sector order loaded: n={len(overview_sector_keys)}" + (f" metric={met_eff}" if met_eff else ""))
    ...
    overview_norm = [_jp_sector_key(x) for x in (overview_sector_keys or []) if _jp_sector_key(x)]
    ordered_norm = reorder_keys_by_overview(existing_keys=existing_norm, overview_keys=overview_norm) if overview_norm else existing_norm
    ...
    print("[JP] sector render order head:", sector_keys[:20])
    ...
    # -------------------------------------------------------------------------
    # 1.5) ✅ Write list.txt in correct order (THIS FIXES THE “STILL WRONG” FEEL)
    # -------------------------------------------------------------------------
    try:
