# scripts/render_images_jp/sector_blocks/layout.py
# -*- coding: utf-8 -*-
...
class LayoutSpec:
    ...
    header_subtitle_line2_y: float = 0.870  # 第二行副標題位置

    footer_y1: float = 0.040
    footer_y2: float = 0.020

    top_box_y0: float = 0.840  # 下移以容納兩行副標題
...
# JP presets
# - jp: default two-line (適合 日股代號+公司名 很長，第二行放狀態/昨況)
# - jp1: one-line variant（如果你未來想把第二行關掉）
PRESETS: Dict[str, LayoutSpec] = {
    "jp": LayoutSpec(two_line=True),
    "jp1": LayoutSpec(two_line=False),
    # 兼容：你 cli.py 可能 default=us（從 CN 複製來的）
