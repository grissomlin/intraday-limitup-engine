...
def normalize_market(m: str) -> Tuple[str, str]:
    """
    Normalize market input to (lower, UPPER).

    ✅ India folder is markets/india/ (NOT markets/in/)
    So we normalize:
      - india / nse / bse / in  -> india
    Keep other markets unchanged.
    """
    ml = (m or "").strip().lower().replace("_", "").replace("-", "")
    ...
    alias = {
        ...
        "nse": "india",
        "bse": "india",
        "in": "india",  # backward compatibility
    }
...
def drive_upload(
    *,
    ...
    payload_path: Optional[Path],  # ✅ NEW
    upload_mode: str,
    images_mode: str,
    workers: int = 8,
) -> None:
    ...
    - latest_<slot>.mp4
    - latest_<slot>_images.zip (or png dir)
    - list.txt (if exists)
    - latest_<slot>.payload.json (optional, if payload_path provided)
    - outputs/latest_meta.json (optional)
    """
    if str(REPO_ROOT) not in sys.path:
        sys.path.insert(0, str(REPO_ROOT))
    ...
    # -----------------
    if want("video"):
        if not out_mp4 or not Path(out_mp4).exists():
    ...
    # -----------------
    if want("images"):
        if not images_dir_path or not Path(images_dir_path).exists():
    ...
    if payload_path is not None:
        ...
        if payload_path.exists():
            ...
            print(f"[drive] payload not found (skip): {payload_path}", flush=True)

    # -------------------------------------------------
    # ✅ latest_meta.json (optional, for dashboard)
    # -------------------------------------------------
