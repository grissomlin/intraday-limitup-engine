# scripts/render_images_common/disclaimer_mpl.py
# -*- coding: utf-8 -*-
"""
...
# =============================================================================
# Font loading / selection (market-aware, avoid KR missing Hangul)
# =============================================================================
...
def _pick_font_family_for_market(market: str) -> str:
    """
    Pick a font family that can render the market's primary languages.
    Priority:
      1) env SUBTITLE_FONT (file path) if provided
      2) market-aware font file candidates (Windows/Linux common)
      3) market-aware installed font family names
      4) fallback DejaVu Sans
    """
    # 1) explicit font file from env
    ...
    # 2) font file candidates by market
    #    (registering file ensures Matplotlib can use it even if not in font cache yet)
    if m == "KR":
        file_candidates = [
            # Noto (if user installed)
            r"C:\Windows\Fonts\NotoSansCJKkr-Regular.otf" if is_win else "",
            r"C:\Windows\Fonts\NotoSansKR-Regular.otf" if is_win else "",
            "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc",
            "/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc",
            # Windows built-in Korean
            r"C:\Windows\Fonts\malgun.ttf" if is_win else "",
            # Optional: Nanum
            "/usr/share/fonts/truetype/nanum/NanumGothic.ttf",
        ]
    elif m == "JP":
        file_candidates = [
            r"C:\Windows\Fonts\YuGothR.ttc" if is_win else "",
            r"C:\Windows\Fonts\YuGothM.ttc" if is_win else "",
            "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc",
            "/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc",
        ]
    else:
        ...
        file_candidates = [
            "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc",
            "/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc",
            r"C:\Windows\Fonts\NotoSansCJK-Regular.ttc" if is_win else "",
            r"C:\Windows\Fonts\msjh.ttc" if is_win else "",
            r"C:\Windows\Fonts\msjhl.ttc" if is_win else "",
    ...
    for fp in file_candidates:
        if not fp:
            continue
        name = _try_register_font_file(Path(fp))
        if name:
            return name

    # 3) installed font family names by market (no file path needed)
    ...
    if m == "KR":
        name_candidates = [
            ...
            "NanumGothic",            # common in Linux
            # fallback that can still render CJK (may lack Hangul, so keep late)
            "Noto Sans CJK TC",
            "Microsoft JhengHei",
            "DejaVu Sans",
        ]
...
def render_disclaimer_page(
    ...
    *,
    ...
    # ✅ market-aware font (fix KR Hangul squares)
    font_family = _pick_font_family_for_market(market)
...
if __name__ == "__main__":
    ...
    render_disclaimer_page(p, market="KR", theme="dark")
    print("written:", p)
