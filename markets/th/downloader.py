# markets/th/downloader.py
# -*- coding: utf-8 -*-
"""
TH pipeline (rolling window, DB-based snapshot builder)

- run_sync(): build/refresh stock list, download prices (rolling window), write DB
...
from .th_db import init_db
from .th_download import bulk_insert_errors, download_batch, download_one_th, insert_prices
from .th_list import get_th_stock_list, get_th_stock_list_from_db
from .th_snapshot import run_intraday
...
def run_sync(start_date: Optional[str] = None, end_date: Optional[str] = None, *, refresh_list: bool = True) -> Dict[str, Any]:
    ...
    log(f"📦 TH DB = {db_path}")
    log(f"🚀 TH run_sync | window: {start_date} ~ {end_date} | refresh_list={refresh_list}")
    ...
    log(f"🧩 list: disable_thaifin={_disable_thaifin()} xlsx_path={_list_xlsx_path()}")

    # list
    if refresh_list:
        items = get_th_stock_list(db_path)
    else:
        items = get_th_stock_list_from_db(db_path)
        if not items:
            items = get_th_stock_list(db_path)
    ...
    pbar = tqdm(batches, desc="TH批次同步", unit="batch")
        ...
        for batch in pbar:
            ...
            if failed_batch and _fallback_single_enabled():
                ...
                for sym in failed_batch:
                    df_one, err_one = download_one_th(sym, start_date, end_excl_date)
    ...
    log(f"📊 TH 同步完成 | 成功:{success} 失敗:{failed}")
    return {
        "success": success,
        ...
        "filters": {
            ...
            "list_xlsx_path": _list_xlsx_path(),
            "disable_thaifin": _disable_thaifin(),
        },
    }
