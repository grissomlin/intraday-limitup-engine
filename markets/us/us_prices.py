# markets/us/us_prices.py
# -*- coding: utf-8 -*-
"""
US rolling-window price sync (DB-based) — Batch + Clean Stats

你要的重點：
✅ 保留對外 API：run_sync(start_date=None, end_date=None, refresh_list=True)
✅ 下載改 batch：yf.download("AAPL MSFT ...", group_by="ticker")
✅ 統計「不灌水」：每個 ticker 最終只算一次 ok/failed（fallback 成功會把 failed 改 ok）
✅ batch error 記錄更乾淨：download_errors 只寫「最終仍失敗」的 ticker（不重複、不洗版）
✅ fallback 寫入更快：executemany + INSERT OR REPLACE
✅ DB schema 不破壞：若表不存在會自動補齊（stock_prices / stock_info / download_errors）

環境變數（建議跟 JP/KR 對齊）：
- US_DB_PATH
- US_ROLLING_TRADING_DAYS        (default 30)
- US_CALENDAR_TICKER             (default ^GSPC)
- US_CAL_LOOKBACK_CAL_DAYS       (default 180)
- US_ROLLING_CAL_DAYS            (default 90)  # calendar 失敗才用
- US_DAILY_BATCH_SIZE            (default 200)
- US_BATCH_SLEEP_SEC             (default 0.05)
- US_FALLBACK_SINGLE             (default 1)
- US_YF_THREADS                  (default 1)
- US_SLEEP_SEC                   (default 0.02)  # 單檔 fallback sleep
"""
...
try:
    # 你 repo 可能已有這些工具
    from .us_calendar import (  # type: ignore
...
try:
    from .us_list import get_us_stock_list  # type: ignore
except Exception:
    get_us_stock_list = None  # type: ignore
...
# =============================================================================
def _db_path() -> str:
    return os.getenv("US_DB_PATH", os.path.join(os.path.dirname(__file__), "us_stock_warehouse.db"))


def _rolling_trading_days() -> int:
    return int(os.getenv("US_ROLLING_TRADING_DAYS", "30"))


def _calendar_ticker() -> str:
    return os.getenv("US_CALENDAR_TICKER", "^GSPC")


def _calendar_lookback_cal_days() -> int:
    return int(os.getenv("US_CAL_LOOKBACK_CAL_DAYS", "180"))


def _fallback_rolling_cal_days() -> int:
    return int(os.getenv("US_ROLLING_CAL_DAYS", "90"))


def _batch_size() -> int:
    return int(os.getenv("US_DAILY_BATCH_SIZE", "200"))


def _batch_sleep_sec() -> float:
    return float(os.getenv("US_BATCH_SLEEP_SEC", "0.05"))


def _fallback_single_enabled() -> bool:
    return str(os.getenv("US_FALLBACK_SINGLE", "1")).strip() == "1"


def _yf_threads_enabled() -> bool:
    return str(os.getenv("US_YF_THREADS", "1")).strip() == "1"


def _single_sleep_sec() -> float:
    return float(os.getenv("US_SLEEP_SEC", "0.02"))
...
def _latest_trading_day_from_calendar(asof_ymd: Optional[str] = None) -> Optional[str]:
    ...
    # internal fallback: use yfinance calendar ticker index
...
# List helpers
# =============================================================================
def _get_us_items(db_path: str, refresh_list: bool) -> List[Tuple[str, str]]:
    """
    回傳 [(symbol, name), ...]
    盡量用 repo 的 us_list.get_us_stock_list；若失敗就用 DB stock_info 退路。
    """
    items: List[Tuple[str, str]] = []

    if get_us_stock_list is not None:
        try:
            # ✅ FIX: us_list.get_us_stock_list expects Path, but we pass str upstream.
            raw = get_us_stock_list(db_path=Path(db_path), refresh_list=refresh_list)
            # 兼容：[(sym,name)] 或 [(sym,name,sector,...)] 或 [{"symbol":...}]
            ...
            log(f"⚠️ us_list.get_us_stock_list failed (fallback DB): {e}")
...
def run_sync(
    ...
    end_date: Optional[str] = None,  # end_exclusive in caller contract (downloader_us.py docstring)
    refresh_list: bool = True,
) -> Dict[str, Any]:
    """
    US rolling-window sync:
    ...
    # ---------- decide window ----------
    # 內部統一用 end_inclusive / end_excl_date
    end_inclusive: str

    if end_date:
        # 使用者給 end_exclusive（依你 downloader_us.py 註解），我們轉成 end_inclusive = end_excl - 1 day（粗略）
        # 但為了不踩假日，仍建議用 calendar 推到 <= (end_excl-1) 的最後交易日
    ...
    # ---------- list ----------
    items = _get_us_items(db_path, refresh_list=refresh_list)
    if not items:
    ...
    log(f"📦 US DB = {db_path}")
    log(f"🚀 US run_sync | window: {start_ymd} ~ {end_inclusive} | refresh_list={refresh_list}")
    ...
    pbar = tqdm(batches, desc="US批次同步", unit="batch")
    ...
    try:
        for batch in pbar:
            ...
            if err_msg:
                # 整批掛：先把 batch 都標成 failed（原因相同），再看 fallback 是否救回
                for sym in batch:
            ...
            # batch 級別：暫時把 ok/fail 標記（還可能被 fallback 改寫）
            failed_batch_set = set(failed_batch or [])
            for sym in batch:
                if sym in failed_batch_set:
                    # 先記 fail reason（之後 fallback 成功會刪掉）
                    final_failed[sym] = "batch_missing_or_no_close"
                else:
                    ok_set.add(sym)
                    # 若先前被標 fail（例如前一批誤判），這裡可清掉
                    if sym in final_failed:
                        final_failed.pop(sym, None)

            # 單檔 fallback：只救 failed_batch（或整批 err 時也可以救，但我們已把整批先標 final_failed）
            if _fallback_single_enabled():
                # 只對「目前在 final_failed 內」且屬於這批的做補抓，避免重複抓
                ...
                for sym in need_fallback:
                    ...
                    if df_one is not None and not df_one.empty:
                        ...
                        conn.commit()
                        ok_set.add(sym)
                        final_failed.pop(sym, None)  # ✅ fallback 救回：移出最終失敗
                    else:
                        # 保留更具體錯誤
                        if err_one:
        ...
        # 寫最終失敗清單（乾淨：只寫一次）
        #（不先清 error table，讓你保留歷史；若你要只留最新一輪，可自己在外層先 DELETE）
    ...
    log(f"📊 US 同步完成 | 成功:{success} 失敗:{failed} / {total}")

    # 依 downloader_us.py 註解：end_date 是 end_exclusive
    return {
        "success": int(success),
        "total": int(total),
        "failed": int(failed),
